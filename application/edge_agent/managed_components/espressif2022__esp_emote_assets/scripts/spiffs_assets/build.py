...
def process_board_icon_collection(icon_collection_dir, assets_dir):
    ...
    for root, dirs, files in os.walk(icon_collection_dir):
        for file in files:
            # icon_collection takes .bin icons plus a small allow-list of
            # animation .eaf that are status/overlay animations (NOT emoji):
            # listen.eaf and the four breathing status circles. These ride
            # the icon path exactly like listen.eaf. An explicit name list
            # is used on purpose: a broad endswith('.eaf') would also pull
            # in the emoji .eaf (angry/Happy/...), which are packed
            # separately as emoji_collection via emote.json -- double-packing
            # them here would bloat the bin and is deliberately avoided.
            if file.lower().endswith(('.bin')) or file.lower() in (
                'listen.eaf', 'status_blue.eaf', 'status_green.eaf',
                'status_yellow.eaf', 'status_red.eaf'):

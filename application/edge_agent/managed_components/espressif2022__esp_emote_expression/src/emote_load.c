...
esp_err_t emote_unload_assets(emote_handle_t handle)
{
    ...
    if (handle->gfx_handle) {
        ...
        for (int i = EMOTE_DEF_OBJ_ANIM_EYE; i < EMOTE_DEF_OBJ_MAX; i++) {
            ...
            // EMOTE_DEF_OBJ_ICON_STATUS is now an anim object (status-cluster
            // redesign) but sits outside the contiguous ANIM_EYE..EMERG_DLG
            // range, so it must be matched explicitly here; freeing it via
            // the image branch would free the wrong union member (heap
            // corruption on unload/reload).
            if ((i >= EMOTE_DEF_OBJ_ANIM_EYE && i <= EMOTE_DEF_OBJ_ANIM_EMERG_DLG)
                    || i == EMOTE_DEF_OBJ_ICON_STATUS) {
                if (entry->data.anim) {
            ...
            } else if (i == EMOTE_DEF_OBJ_ICON_CHARGE) {
                if (entry->data.img) {

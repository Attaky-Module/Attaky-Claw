...
static const obj_creation_entry_t obj_creation_table[] = {
    ...
    { EMOTE_DEF_OBJ_ICON_STATUS,    emote_create_anim_obj,  emote_config_anim_obj          },
...
static void emote_status_timer_callback(void *data)
{
    ...
    if (handle) {
        // Status timer drives the clock only. Battery/charge render path
        // was removed with the status-cluster redesign (status slot is
        // now a state-driven anim object, no battery_label/charge_icon).
...
esp_err_t emote_apply_fonts(emote_handle_t handle, const uint8_t *fontData)
{
    ...
    /* Attaky Claw: runtime-loaded LVGL font binary via
     * gfx_font_lv_load_from_binary() renders as pixel-noise garbage on
     * the scrolling toast_label widget (every char becomes a corrupt
     * glyph). Clock and default labels use the bundled C-source
     * font_maison_neue_book_26 / font_puhui_basic_20_4 and render
     * cleanly. Skip the runtime override and leave toast on the bundled
     * font set in emote_config_label_toast_obj() — Latin-coverage is
     * fine for the LLM/stage notification content shown there. */
    ESP_LOGI(TAG, "emote_apply_fonts: skipping runtime font override (using bundled font for toast)");
    (void)fontData;

...
static void **emote_get_cache_ptr_by_obj_type(emote_handle_t handle, emote_obj_type_t obj_type)
{
    ...
    case EMOTE_DEF_OBJ_ICON_STATUS:
        // Status slot is now a state-driven anim object (status-cluster
        // redesign), so it allocates the anim cache like the other anims.
    ...
    // EMOTE_DEF_OBJ_ICON_CHARGE is intentionally retained but currently
    // unreachable: the charge_icon layout object was removed in the
    // status-cluster redesign, so this branch is never created or hit.
    // Kept to keep the vendored patch minimal and ease future upstream
    // re-merge.
...
// emote_get_img_dsc_by_obj_type() and emote_set_icon_image() were removed
// with the status-cluster redesign. The status slot is now a state-driven
// anim (no img_dsc), and the only other image consumers (battery_bg /
// charge_icon) were retired along with their layout objects, so the
// image-write helper has no remaining callers. The anim path
// (emote_set_icon_animation) and emote_get_cache_ptr_by_obj_type remain.
...
static esp_err_t emote_handle_listen_event(emote_handle_t handle, const char *message)
{
    ...
    // Legacy status-icon image write (icon_mic) retired: the status slot
    // is now a state-driven anim driven via emote_set_status_state().
    // The listen_anim overlay above is unaffected.
    return ret;
}
...
static esp_err_t emote_handle_speak_event(emote_handle_t handle, const char *message)
{
    ...
    // Legacy status-icon image write (icon_speaker) retired: the status
    // slot is now a state-driven anim driven via emote_set_status_state().
...
static esp_err_t emote_handle_sys_set_event(emote_handle_t handle, const char *message)
{
    ...
    // Legacy status-icon image write (icon_tips) retired: the status slot
    // is now a state-driven anim driven via emote_set_status_state().
...
{
    // Battery/charge render path removed with the status-cluster redesign:
    // the status slot is now a state-driven anim object, and the
    // battery_label / charge_icon layout objects no longer exist. This
    // remains a callable no-op so existing callers (idle event, BAT event)
    // do not brick; bat_is_charging / bat_percent are still tracked for
    // any future consumer but are not rendered here.
    if (!handle) {
...
{
    // charge_icon layout object removed with the status-cluster redesign.
    // Callable no-op (never bricks); no dead asset lookup, no log spam.
    if (!handle) {
        return ESP_ERR_INVALID_ARG;
    }
    return ESP_OK;
...
{
    // charge_icon layout object removed: silent no-op.
    (void)handle;
}

esp_err_t emote_set_bat_status_label(emote_handle_t handle)
{
    // battery_label layout object removed with the status-cluster
    // redesign; rendering it would fall back to the (also-deleted)
    // default label and warn. Callable no-op (never bricks).
    if (!handle) {
        return ESP_ERR_INVALID_ARG;
    }
    return ESP_OK;
...
esp_err_t emote_set_status_state(emote_handle_t handle, status_state_t state)
{
    if (!handle) {
        return ESP_ERR_INVALID_ARG;
    }
    ...
    switch (state) {
    case STATUS_STATE_NORMAL:
        name = "status_blue";
        break;
    case STATUS_STATE_RECORDING:
        name = "status_green";
        break;
    case STATUS_STATE_SPEAKING:
        name = "status_yellow";
        break;
    case STATUS_STATE_NOTIFICATION:
        name = "status_red";
        break;
    default:
        ESP_LOGW(TAG, "Unknown status state %d", (int)state);
        return ESP_ERR_INVALID_ARG;
    }

    // The status slot is a state-driven anim object. Reuse the proven
    // obj-type-generic anim path (same as listen_anim). fps=12/loop=true
    // with the 36-frame breathing assets gives loop = frames/fps =
    // 36/12 = 3.0s per breath: a calm, slow cadence with smooth 12 fps
    // playback (36 frames = fine brightness/scale steps per hardware-
    // visual feedback "needs more frames / smoother"), below
    // the 15 fps listen overlay for a steady ambient indicator.
    esp_err_t ret = emote_set_icon_animation(handle, EMOTE_DEF_OBJ_ICON_STATUS, name, 12, true);
    if (ret != ESP_OK) {
        // Engine-patch path failure must not brick boot: log and continue.
        ESP_LOGW(TAG, "Failed to set status state anim \"%s\": %s",
...
esp_err_t emote_set_event_msg(emote_handle_t handle, const char *event, const char *message)
{
    ...
    if (!entry->skip_hide_ui) {
        ...
        // ICON_STATUS is intentionally NOT swept hidden here. After the
        // status-cluster redesign the status slot is a PERSISTENT,
        // state-driven indicator whose visibility is owned solely by
        // emote_set_status_state(). Unlike the transient event-driven UI
        // (listen/clock/toast/qrcode) it must survive every per-event
        // hide-all -- hiding it here and only restoring it on a state
        // re-drive is exactly what made the status circle never persist.

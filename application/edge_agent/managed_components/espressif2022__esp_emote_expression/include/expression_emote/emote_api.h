...
/**
 * @brief Status indicator state
 *
 * Drives the top-left status slot, which is a state-driven animation
 * object. Each state maps to a colored breathing animation asset:
 *   NORMAL       -> blue   (idle / connected)
 *   RECORDING    -> green  (listening / capturing audio)
 *   SPEAKING     -> yellow (speaking / TTS playback)
 *   NOTIFICATION -> red    (attention / notification)
 */
typedef enum {
    STATUS_STATE_NORMAL = 0,
    STATUS_STATE_RECORDING,
    STATUS_STATE_SPEAKING,
    STATUS_STATE_NOTIFICATION,
} status_state_t;
...
/**
 * @brief Set the status indicator state
 *
 * Maps the state to a colored breathing animation on the status slot
 * (status_blue / status_green / status_yellow / status_red).
 *
 * @param handle Handle to emote manager
 * @param state Status state (see status_state_t)
 * @return ESP_OK on success, error code on failure
 */

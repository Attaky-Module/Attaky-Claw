...
这种「薄包装（thin wrapper）」模式在 Attaky Claw 中很常见：底层功能由 `claw_*` 模块实现，`cap_*` 组件负责将其转化为工具接口和调用路由。
...
| `activate_skill` | 为当前 session 激活 Skill，同步开放对应 `metadata.cap_groups` |
...
// activate_skill 成功响应
{
  ...
  "skill_id": "cap_lua",
  "ok": true,
  "active_skill_ids": ["cap_lua", "cap_files"]
}
```

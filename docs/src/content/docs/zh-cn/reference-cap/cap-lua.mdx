...
description: Lua 脚本引擎在 Attaky Claw 中的定位、cap_lua 工具集，以及同步/异步执行机制。
---
...
## Lua 在 Attaky Claw 中的定位

Attaky Claw 将 **Lua** 作为设备端「可编程自动化」的首选语言。它在系统中扮演三个角色：
...
Attaky Claw 使用 Lua 解释器，通过 `cap_lua_runtime_*` 系列函数初始化和执行脚本。运行时在 `cap_lua_group_init` 中初始化，锁定模块注册后启动。

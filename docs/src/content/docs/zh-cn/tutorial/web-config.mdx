...
电脑或手机需与开发板在 **同一局域网**，或连接至**开发板提供的 SoftAP 热点**（`edge_agent` 中默认的 SSID 为 `attaky-claw-xxxx`，`xxxx` 为设备 MAC 地址）。

在浏览器访问：
[http://attaky-claw.local/](http://attaky-claw.local/)
或
`http://<设备IP>/`
...
在与 Attaky Claw 对话后，你可以点击「Refresh」按钮或「Refresh All」按钮，刷新记忆文件，查看最新的记忆内容。
...
Attaky Claw 运行时的文件系统结构见 [文件系统布局](@lang/reference-project/boot-and-runtime#文件系统)。

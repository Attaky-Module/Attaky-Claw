...
**Typical causes:**
- Attaky Claw never receives IM traffic or cannot call the IM API to send.
- LLM runtime failure.
- Event Router rules mis-route messages.

**What to try:**
1. Check HTTP status and error text in logs.
2. Confirm the IM message was ingested—serial logs should show it. [➡️ Attaky Claw cannot send/receive IM](#attaky-claw-cannot-send-or-receive-im)
...
### Attaky Claw cannot send or receive IM

...
Attaky Claw is an **event-driven** Agent framework: an **Event** is the trigger for Agent messages and automation.

<LinkCard href="@lang/reference-project/core-cap-event" title="Core, Capability, and Event" description="Events as the message carrier in Attaky Claw" />

## Definitions

### Event

An Event is the message carrier in Attaky Claw: id, source, target, type, chat id, sender, text or JSON payload, timestamp, session policy, and more.
...
`router_rules.json` is Attaky Claw's automation rules file: it defines how Events that enter `claw_event_router` are handled.
...
To auto-publish `agent_stage` events, adjust this menuconfig option: <kbd>(Top)</kbd> → <kbd>Component config</kbd> → <kbd>Attaky Claw Core</kbd> → <kbd>Agent stage notification verbosity</kbd>

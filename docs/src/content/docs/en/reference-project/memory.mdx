...
LLMs are stateless—each request starts fresh. Attaky Claw uses `claw_memory` to inject two kinds of persisted context before each request so the Agent appears to “remember” chats and users.

The two memories target different horizons and solve different problems:

## Memory modes

Attaky Claw's memory module provides two modes:
...
Attaky Claw scopes history by **Session ID**. Turns under the same Session ID form one thread; **changing Session ID switches to another isolated thread**—ideal for parallel chats from different IM users or channels.

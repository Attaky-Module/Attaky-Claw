...
<LinkToSource relativePath="application/edge_agent/main/main.c" /> initializes required system pieces and peripherals so the Attaky Claw Agent can run.
...
3. **Mount FATFS**
    ...
    If mount fails, `ESP_ERROR_CHECK` triggers `abort` and the device reboots—a filesystem is required for Attaky Claw to run normally.
...
6. **Start Attaky Claw Agent stack**

...
Attaky Claw `edge_agent` stores boot defaults in menuconfig (`sdkconfig`) and runtime overrides in NVS.
...
Attaky Claw can connect QQ, Feishu, Telegram, and WeChat ClawBot, with extension hooks. If credentials for a channel are empty, that IM cannot work end-to-end.
...
Search API keys are optional. If both are empty, Attaky Claw cannot fetch online results via search.

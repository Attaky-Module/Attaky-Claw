...
description: Where Lua fits in Attaky Claw, the `cap_lua` tool surface, and sync vs async execution.
---
...
Attaky Claw uses **Lua** as the primary on-device language for **programmable automation**. It plays three roles:
...
Attaky Claw runs Lua via `cap_lua_runtime_*` helpers. The runtime initializes in `cap_lua_group_init`, locks module registration, then starts.

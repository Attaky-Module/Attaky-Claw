...
A **capability** is the uniform abstraction for anything callable in Attaky Claw. Each `cap_*` component registers one or more metadata-rich **descriptors** (`claw_cap_descriptor_t`) with `claw_cap`, and all execution funnels through the shared dispatcher.

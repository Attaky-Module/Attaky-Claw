...
/* Optional read-only observer fired EXACTLY ONCE per real inbound event
 * reaching the processing path, BEFORE routing/rule evaluation. The
 * event pointer is valid only for the duration of the call; the
 * observer must not retain it, free it, or mutate it. It is a passive
 * tap (UI status hint) -- it cannot influence routing. Self-emitted
 * router events (source_cap == "claw_event_router") do NOT fire it.
 * Single slot: a second registration replaces the first; cb == NULL
 * unregisters. */
typedef void (*claw_event_router_observer_fn)(const claw_event_t *event,
                                              void *ctx);
...
/* Register/clear the passive inbound observer (see
 * claw_event_router_observer_fn). Safe to call before or after
 * init/start. Intended for ONE-TIME registration at app bring-up;
 * not designed for concurrent re-registration from multiple tasks
 * (the slot is updated lock-free -- see the .c rationale). */

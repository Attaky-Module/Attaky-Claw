...
/* Passive inbound observer (Task 4.2). File-scope so it survives across
 * init/stop and may be registered before init. Set rarely (once at app
 * bring-up); read on the single router task. Plain pointers, no lock --
 * a torn read at worst skips/duplicates one UI status hint, never
 * affects routing. */
static claw_event_router_observer_fn volatile s_observer_cb = NULL;
static void *volatile s_observer_ctx = NULL;
                                                 ...
                                                 claw_event_router_result_t *out_result)
{
    ...
    /* Passive UI status tap (Task 4.2): fire once per real inbound
     * event, before any rule/route work, so a missing/failed observer
     * cannot change routing. Read-only contract documented in the
     * header. No-op + byte-identical path when unregistered. Called
     * WITHOUT the router lock held: the observer must not assume
     * router-internal state is quiesced and must not re-enter router
     * APIs. cb+ctx are snapshotted into locals so the compiler cannot
     * reload them across the call; this is NOT an atomic pair -- two
     * volatile reads give no cross-variable coherence. Correctness
     * relies on the documented invariant that set_observer() completes
     * once at app bring-up BEFORE any event processing begins (see the
     * header); concurrent re-registration while the router runs is
     * unsupported and could transiently pair a new cb with an old
     * ctx. */
    claw_event_router_observer_fn observer_cb = s_observer_cb;
    void *observer_ctx = s_observer_ctx;
    if (observer_cb != NULL) {
        observer_cb(event, observer_ctx);
    }
...
void claw_event_router_set_observer(claw_event_router_observer_fn cb, void *ctx)
{
    s_observer_cb = cb;
    s_observer_ctx = ctx;
}

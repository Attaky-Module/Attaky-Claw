...
/* Return the largest N <= max_bytes such that s[0..N) ends on a UTF-8
 * character boundary (no truncation in the middle of a multi-byte
 * sequence). Used when slicing LLM tool-call arg strings for the on-
 * screen toast: a byte-level cut on Chinese / emoji / other multi-byte
 * codepoints would surface as a garbled glyph in the scrolling label. */
static size_t utf8_safe_truncate_len(const char *s, size_t max_bytes)
{
    if (!s) {
        return 0;
    }
    size_t actual = strnlen(s, max_bytes + 4);
    if (actual <= max_bytes) {
        return actual;
    }
    /* Walk backwards from max_bytes while we are inside a UTF-8
     * continuation byte (0b10xxxxxx). When we land on either a 1-byte
     * ASCII char (0xxxxxxx) or a multi-byte lead (11xxxxxx), the
     * preceding [0..max_bytes) is a clean prefix. */
                                     ...
                                     uint32_t iteration)
{
    ...
    for (i = 0; i < response->tool_call_count; i++) {
        ...
        if (args && args[0]) {
            size_t arg_show = utf8_safe_truncate_len(args, 40);
            const char *ellipsis = (strlen(args) > arg_show) ? "..." : "";
            written = snprintf(buf + off, sizeof(buf) - off, "%s%s(%.*s%s)",
                               i == 0 ? "" : ", ", name,
                               (int)arg_show, args, ellipsis);
        } else {

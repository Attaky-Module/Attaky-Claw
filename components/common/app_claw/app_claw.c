...
/* Inbound-event observer feeding the status_state aggregator
 * (design section 2 / section 6, plan Task 4.3).
 *
 * NOTIFICATION trigger: design section 2 -- "inbound IM message OR
 * agent/scheduler-initiated reminder". The router-level observer
 * (claw_event_router_observer_fn) is documented to fire EXACTLY ONCE
 * per real inbound event reaching the processing path, BEFORE
 * routing, and self-emitted router events (source_cap ==
 * "claw_event_router") are already excluded by the router. The router
 * tap still also delivers internal/system triggers (e.g. the app boot
 * "startup" event, source_cap "app_claw"), which are NOT user-facing
 * notifications, so this observer applies a positive allowlist in its
 * body (see the in-function comment) to keep only genuine inbound IM
 * messages + scheduler/agent reminders per design section 6 -- without
 * it the boot event would drive the circle red on first boot.
 *
 * CLEAR trigger: design section 2 -- NOTIFICATION clears "when the
 * item is processed / the device enters interaction". esp-claw has no
 * clean single observable "processed / entered-interaction" point at
 * this layer: the router exposes only this pre-routing observer (no
 * post-handling / reply callback API), the emote face is not driven
 * per inbound anywhere in the firmware today, and modifying the
 * committed router / status_state source is out of scope. Per the
 * plan's explicit allowance (Task 4.3 Step 1 / Risks: "if no clean
 * single point exists, pick a documented proxy ... do not invent an
 * event"), the documented proxy is: "the next real inbound = the
 * previous item is considered processed". Concretely, the observer
 * firing IS the inbound -> processing boundary (the router processes
 * the event synchronously right after this call on the same task), so
 * each real inbound clears any prior pending NOTIFICATION (the previous
 * item is now superseded -- the device has moved on to handle this one)
 * and then re-raises NOTIFICATION for the current inbound.
 *
 * CONSEQUENCE of this clear-then-set-per-inbound proxy (spec-permitted,
 * plan Risks ~line 249; stated in full here so the closeout / commit
 * body can quote it): a SINGLE notification does NOT auto-return the
 * circle to blue. With no following inbound there is no clear event,
 * so the circle STAYS red and merely re-pulses red on the NEXT inbound.
 * Returning to NORMAL/blue happens ONLY via a higher-precedence state
 * -- the aggregator's own precedence where entering RECORDING/SPEAKING
 * clears a pending NOTIFICATION (deferred ASR/voice work, no caller
 * here yet) -- or via a future real "processed / clear" hook if one is
 * later added. The plan Step 3 hardware-gate phrasing "returns to blue
 * after the message is processed" is therefore NOT literally satisfied
 * by a lone notification under this proxy; this is the accepted,
 * plan-permitted trade-off (Risks line ~249), not a defect, and the
 * behaviour is deliberately left unchanged. RECORDING / SPEAKING are
 * reserved for the deferred ASR/voice work and have no caller here
 * today.
 *
 * Thread: this observer runs on the status-state setter's caller
 * thread -- normally the claw_event_router task, but also any caller
 * of the synchronous claw_event_router_handle_event() (which reaches
 * the observer on its OWN thread). That is safe because the emote-side
 * status-state hook brackets its latch + refresh in emote_lock, making
 * the actual thread identity immaterial; this observer itself only
 * calls the aggregator setters (the aggregator's documented
 * single-writer).
 *
 * Contract: read-only, must not retain/free/mutate the event, must
 * not re-enter router APIs, registered exactly once at bring-up. The
 * status_state setters are the aggregator's single-writer; this
 * observer is that single writer. */
static void app_claw_inbound_status_observer(const claw_event_t *event,
                                              void *ctx)
{
    (void)ctx;
    ...
    /* Positive allowlist for a NOTIFICATION-worthy "real inbound"
     * (design section 6: inbound IM messages + scheduler/agent-initiated
     * reminders). Keyed on router/scheduler-HARDCODED invariants, not
     * user-tweakable schedule config:
     *   - inbound IM message: claw_event_router_publish_message() always
     *     stamps event_type == "message" (router-internal constant).
     *   - scheduler/agent reminder: cap_scheduler_publish_entry_locked()
     *     always stamps source_cap == "cap_scheduler" (the per-item
     *     event_type is user-configurable, so it is NOT a safe key).
     * Everything else is excluded -- in particular the app boot trigger
     * (source_cap "app_claw", event_type "startup") and router
     * self-emitted events (source_cap "claw_event_router", already
     * filtered out by the router observer tap before we are called).
     * Without this guard the boot startup event would drive the circle
     * red on first boot with no real inbound, breaking NORMAL-by-default. */
    bool is_inbound_message =
        strcmp(event->event_type, "message") == 0;
    bool is_scheduler_reminder =
        strcmp(event->source_cap, "cap_scheduler") == 0;
    if (!is_inbound_message && !is_scheduler_reminder) {
        return;
    }

    /* Documented proxy (plan Risks ~line 249): "next real inbound =
     * previous item considered processed". Clear then re-raise so each
     * inbound is a fresh NOTIFICATION pulse. Consequence, stated so the
     * closeout/commit body can quote it: a LONE notification stays red
     * and only re-pulses on the next inbound -- it does NOT auto-return
     * to blue. NORMAL/blue is reached only via a higher-precedence state
     * (RECORDING/SPEAKING, deferred) or a future real clear hook. This
     * is the accepted, spec-permitted trade-off; behaviour is left
                         ...
                         const app_claw_storage_paths_t *paths)
{
    ...
    /* One-time, at bring-up, after init and before start (matches the
     * observer's documented single-registration contract): tap real
     * inbound IM / scheduler-agent events to drive the status circle. */
    claw_event_router_set_observer(app_claw_inbound_status_observer, NULL);
#if CONFIG_APP_CLAW_CAP_SCHEDULER

...
#define CONFIG_APP_WIFI_AP_SSID_PREFIX "attaky-claw"
#endif
...
static char s_sta_current_ssid[33];  /* SSID reported by esp_wifi_sta_get_ap_info(); empty when not joined */
...
void wifi_manager_get_status(wifi_manager_status_t *status)
{
    ...
    /* Best-effort populate the currently-joined SSID. Fail-soft: any
     * error → NULL.
     *
     * Concurrency: unlike s_ip_addr / s_ap_ip / s_ap_ssid (written
     * only from the Wi-Fi event task), s_sta_current_ssid is written
     * here, which can be called from any task. Concurrent get_status
     * callers may observe a torn buffer mid-memcpy; the worst case
     * is one status snapshot returning a NUL-terminated chimera of
     * two SSIDs. Accepted for a best-effort display field — escalate
     * to a mutex or event-task funnel only if torn reads become a
     * functional problem. */
    status->sta_ssid = NULL;
    if (s_connected) {
        wifi_ap_record_t ap_record = {0};
        esp_err_t info_err = esp_wifi_sta_get_ap_info(&ap_record);
        if (info_err == ESP_OK) {
            memcpy(s_sta_current_ssid, ap_record.ssid, sizeof(s_sta_current_ssid) - 1);
            s_sta_current_ssid[sizeof(s_sta_current_ssid) - 1] = '\0';
            if (s_sta_current_ssid[0] != '\0') {
                status->sta_ssid = s_sta_current_ssid;
            }
        } else {
            ESP_LOGD(TAG, "esp_wifi_sta_get_ap_info failed: %s", esp_err_to_name(info_err));
        }
    }
}

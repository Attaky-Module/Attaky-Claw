...
static const char *display_arbiter_owner_to_str(display_arbiter_owner_t owner)
{
    ...
    case DISPLAY_ARBITER_OWNER_EMOTE:
        return "emote";
    case DISPLAY_ARBITER_OWNER_SETUP:
        return "setup";
    default:
        return "unknown";
    }
...
esp_err_t display_arbiter_acquire_setup(void)
{
    ...
    if (s_state.owner != DISPLAY_ARBITER_OWNER_EMOTE) {
        ESP_LOGW(TAG, "setup acquire requires emote owner (got %s)", display_arbiter_owner_to_str(s_state.owner));
        ret = ESP_ERR_INVALID_STATE;
        goto fail;
    }

    ESP_GOTO_ON_ERROR(display_arbiter_change_owner_locked(DISPLAY_ARBITER_OWNER_SETUP), fail, TAG,
                      "switch to setup owner failed");
    owner_changed = true;

fail:
    display_arbiter_unlock();
    if (ret == ESP_OK && owner_changed) {
        display_arbiter_notify_owner_changed(DISPLAY_ARBITER_OWNER_SETUP);
    }
    return ret;
}
...
esp_err_t display_arbiter_release_setup(void)
{
    ...
    ESP_GOTO_ON_FALSE(s_state.owner == DISPLAY_ARBITER_OWNER_SETUP, ESP_ERR_INVALID_STATE, fail, TAG,
                      "setup owner is not active");
...
fail:
    display_arbiter_unlock();
    if (ret == ESP_OK && owner_changed) {
        display_arbiter_notify_owner_changed(DISPLAY_ARBITER_OWNER_EMOTE);
    }
    return ret;
}
